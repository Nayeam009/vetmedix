...
export const AdminMobileNav = ({ pendingOrders = 0, pendingVerifications = 0 }: AdminMobileNavProps) => {
  ...
  const sectionsWithBadges = navSections.map(section => ({
    ...
    return (
      <div className="flex h-full max-h-[100dvh] flex-col bg-background overflow-hidden">
        {/* Logo */}
        <div className="sticky top-0 z-10 p-4 sm:p-5 border-b border-border/50 bg-gradient-to-r from-primary/5 to-accent/5">
          <SheetClose asChild>
        ...
        {/* Navigation */}
        <nav className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3 sm:p-4 space-y-4">
        ...
        {/* Back to Site */}
        <div className="sticky bottom-0 z-10 p-3 sm:p-4 pb-[calc(env(safe-area-inset-bottom)+12px)] border-t border-border/50 bg-muted/30">
          <SheetClose asChild>
            <Link
              to="/"
