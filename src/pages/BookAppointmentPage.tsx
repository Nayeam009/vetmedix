...
import { Calendar, Clock, ArrowLeft, Loader2, User, Info } from 'lucide-react';
...
const BookAppointmentPage = () => {
  ...
  const getAvailableTimeSlots = (): string[] => {
    if (!formData.date || !formData.doctorId) {
      // Return default slots if no doctor selected
    ...
    if (!selectedDoctor?.schedules?.length) {
      // No schedule configured, return default slots
      return ['09:00 AM', '10:00 AM', '11:00 AM', '02:00 PM', '03:00 PM', '04:00 PM'];
    }
    ...
    return daySchedules.map((s: any) => s.start_time);
  };
  ...
  const isDateAvailable = (dateStr: string): boolean => {
    ...
    const selectedDoctor = doctorsWithSchedules.find((d: any) => d.id === formData.doctorId);
    if (!selectedDoctor?.schedules?.length) return true;
      ...
      <main className="container mx-auto px-4 py-8 max-w-xl">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Fill in the details to schedule your visit</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input 
                    ...
                    required 
                  />
                  {formData.date && formData.doctorId && !isDateAvailable(formData.date) && (
                    <p className="text-xs text-amber-600">Doctor not available on this day</p>
                  )}
                </div>
                <div className="space-y-2">
                  <Label>Time</Label>
                  ...
                  {availableTimeSlots.length === 0 && formData.date && (
                    <p className="text-xs text-destructive">No available slots for this date</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  required 
                />
                {errors.petName && <p className="text-sm text-red-500">{errors.petName}</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  className="w-full h-24 px-3 py-2 rounded-lg border border-border bg-background resize-none text-sm" 
                />
